...
function loadStatsPage()
{
	...
	{//Outer for loop iterates over each row, dates dont matter
		oppTeamName = table.rows[row_counter].cells[1].innerHTML;
		homeTeamScr = parseInt(table.rows[row_counter].cells[2].innerHTML);
		oppTeamScr = parseInt(table.rows[row_counter].cells[3].innerHTML);
			if(homeTeamScr > oppTeamScr)
			{
				winNum++;
			...
			{
				lossNum++;
				table.rows[row_counter].cells[4].innerHTML = oppTeamName;
			}
			var winner = table.rows[row_counter].cells[4].innerHTML;
	}
...
{
	//make an achor with a for loop
	//$ means javascript identifiers for an object
	//innerHTML - property can be used along with getElementByID() within your JavaScript code to refer to an HTML element and change its contents
	var select = ""
	for(var i = 0; i < players.length; i++)
	{
		var player = players.indexOf(i)
		select = select + `<a class='dropdown-item' href='#' onclick='switchPlayers(${i})'> ${players[i].name} </a>`
	}	
